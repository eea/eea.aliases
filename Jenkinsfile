      ...
      steps {
        parallel(
          ...
          "CSS Lint": {
            node(label: 'docker') {
              script {
                sh '''docker pull eeacms/csslint; docker run -i --rm --name="$BUILD_TAG-csslint" -e GIT_SRC="https://github.com/eea/$GIT_NAME.git" -e GIT_NAME="$GIT_NAME" -e GIT_BRANCH="$BRANCH_NAME" -e GIT_CHANGE_ID="$CHANGE_ID" eeacms/csslint'''
              }
            }
          },
